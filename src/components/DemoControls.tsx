    ...
    <section className="p-5 bg-card/80 backdrop-blur-sm border border-border rounded-lg">
      ...
      <div className="flex items-center justify-between pt-4 border-t border-border">
        <div className="flex items-center gap-3">
          <span className="text-sm text-neutral-300">Auto Response</span>
          <button
            onClick={onAutoResponseToggle}
            className={`relative w-11 h-6 rounded-full transition-colors ${
              autoResponse ? 'bg-primary' : 'bg-muted'
            }`}
          >
            <div
              className={`absolute top-1/2 -translate-y-1/2 left-0.5 w-5 h-5 rounded-full bg-neutral-300 transition-transform ${
                autoResponse ? 'translate-x-5' : 'translate-x-0'
              }`}
            />
          </button>
          <span className={`text-sm font-medium ${autoResponse ? 'text-white' : 'text-neutral-400'}`}>
            {autoResponse ? 'ON' : 'OFF'}
          </span>
        </div>
      </div>
